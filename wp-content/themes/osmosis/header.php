...
<!--<![endif]-->
	...
	<body id="grve-body" <?php body_class(); ?>>
		<?php do_action( 'grve_body_top' ); ?>

		<?php

			$grve_feature_data = grve_get_feature_data();
			$grve_header_class = $grve_feature_data['header_style'];
			...
			if ( 'logo-top' == $grve_header_mode ) {
				$grve_logo_align = grve_option( 'logo_top_align', 'center' );
				$grve_header_class .= ' grve-logo-top';
			}
			...
			if ( empty( $grve_menu_type ) ) {
				$grve_menu_type = 'simply';
			}
			...
			if ( 'logo-top' == $grve_header_mode ) {
				$grve_sticky_header_type = 'simply';
			}
		...
		<?php
			...
			$grve_menu_responsive_align = grve_option( 'menu_responsive_align', 'left' );

			//Responsive Menu Classes
			$grve_menu_responsive_classes = array();

			$grve_menu_responsive_classes[] = 'grve-style-' . $grve_menu_responsive_style;
			$grve_menu_responsive_classes[] = 'grve-align-' . $grve_menu_responsive_align;

			$grve_menu_responsive_class_string = implode( ' ', $grve_menu_responsive_classes );

		?>

		<!-- Responsive Menu -->
		<nav id="grve-main-menu-responsive" class="<?php echo esc_attr( $grve_menu_responsive_class_string ); ?>">
		...
		<div id="grve-theme-wrapper">
			...
			<header id="grve-header" data-gotosection="<?php echo esc_attr( $grve_go_to_section ); ?>" data-fullscreen="<?php echo esc_attr( $grve_feature_data['data_fullscreen'] ); ?>" data-overlap="<?php echo esc_attr( $grve_feature_data['data_overlap'] ); ?>" data-sticky-header="<?php echo esc_attr( $grve_sticky_header ); ?>" data-logo-background="<?php echo esc_attr( $grve_logo_background ); ?>" data-logo-align="<?php echo esc_attr( $grve_logo_align ); ?>" data-menu-align="<?php echo esc_attr( $grve_menu_align ); ?>" data-menu-type="<?php echo esc_attr( $grve_menu_type ); ?>" data-safebutton="<?php echo esc_attr( $grve_safe_button ); ?>" data-topbar="<?php echo esc_attr( $grve_top_bar ); ?>" data-menu-options="<?php echo esc_attr( $grve_header_menu_options ); ?>" data-header-position="<?php echo esc_attr( $grve_feature_data['data_header_position'] ); ?>" data-backtotop="<?php echo esc_attr( $grve_back_to_top ); ?>" class="<?php echo esc_attr( $grve_header_class ); ?>">
				...
				<div id="grve-inner-header">
					...
					<?php if ( 'logo-top' == $grve_header_mode ) { ?>
					<!-- Top Header -->
					<div class="grve-top-header">
					<?php } ?>

						<div class="grve-container">
							<?php grve_print_header_logo(); ?>
					<?php if ( 'logo-top' == $grve_header_mode ) { ?>
						</div>
					</div>
					<!-- End Top Header -->
					...
					<div class="grve-bottom-header">
						<div class="grve-container">
							<?php grve_print_header_logo('responsive'); ?>
					<?php } ?>
					...
					?>
						...
						<?php do_action( 'grve_header_menu_before' ); ?>
						...
						<?php } else {?>
							<div class="grve-responsive-menu-text">
								<?php echo esc_html( $grve_responsive_menu_text ); ?>
							</div>
						<?php } ?>
						...
						<?php do_action( 'grve_header_menu_after' ); ?>
						<!-- End Main Menu -->

					<?php
						} else {
							do_action( 'grve_header_container_custom_menu_integration' );
							if ( class_exists('UberMenu') && 'ubermenu-container' == grve_option( 'menu_header_integration', 'default' )) {
								echo '<nav id="grve-main-ubermenu">';
								grve_header_ubermenu_nav();
								echo '</nav>';
							}
						}
					?>
