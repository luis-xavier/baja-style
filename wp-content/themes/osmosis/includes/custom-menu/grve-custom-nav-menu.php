 ...
 */
function grve_edit_walker( $walker,$menu_id ) {
	return 'Grve_Walker_Nav_Menu_Edit';
}
if ( version_compare( $wp_version, '5.4', '<' ) ) {
	add_filter( 'wp_edit_nav_menu_walker', 'grve_edit_walker', 10, 2 );
}
 ...
 */
function grve_get_custom_nav_menu_items( $menu_item ) {
...
add_filter( 'wp_setup_nav_menu_item', 'grve_get_custom_nav_menu_items' );

/**
 * Function to update custom menu items
 */
function grve_update_custom_nav_menu_items( $menu_id, $menu_item_db_id, $args ) {

	$custom_nav_menu_fields = array( 'megamenu', 'link_mode', 'link_classes', 'label_text', 'icon_fontawesome' );
...
add_action( 'wp_update_nav_menu_item', 'grve_update_custom_nav_menu_items', 10, 3 );
