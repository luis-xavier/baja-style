    ...
    class ReduxFramework_custom_fonts {
        ...
        public function render () {
            ...
            if ( !empty ( $this->field[ 'custom_fonts' ] ) ) {
                foreach ( $this->field[ 'custom_fonts' ] as $section => $fonts ) {
                    ...
                    echo '<h3>' . $section . '</h3>';
                    echo '<div class="font-error" style="display: none;"><p><strong>' . esc_html__( 'Error', 'osmosis' ) . '</strong>: <span></span></p></div>';
                    ...
                    echo '</tbody></table>';
                    echo '<div class="upload_button_div"><span class="button media_add_font" data-nonce="' . $nonce . '" id="' . $this->field[ 'id' ] . '-custom_fonts">' . esc_html__ ( 'Add Font', 'osmosis' ) . '</span></div><br />';
                }
            } else {
                echo "<h3>" . esc_html__ ( 'No Custom Fonts Found', 'osmosis' ) . "</h3>";
                echo '<div class="upload_button_div"><span class="button media_add_font" data-nonce="' . $nonce . '" id="' . $this->field[ 'id' ] . '-custom_fonts">' . esc_html__ ( 'Add Font', 'osmosis' ) . '</span></div>';
            }
        ...
        public function enqueue () {
            ...
            wp_localize_script( 'redux-field-custom_fonts-js', 'redux_custom_fonts_l10', apply_filters( 'redux_custom_fonts_localized_data', array(
                'delete_error'  => __('There was an error deleting your font:', 'osmosis'),
                'unzip'         => __('Unzipping archive and generating any missing font files.', 'osmosis'),
                'convert'       => __('Converting font file(s)...', 'osmosis'),
                'partial'       => __('The only file(s) imported were those uploaded.  Please refresh the page to continue (making note of any errors before doing so, please).', 'osmosis'),
                'unknown'       => __('An unknown error occurred. Please try again.', 'osmosis'),
                'complete'      => __('Conversion complete.  Refreshing page...', 'osmosis'),
                'media_title'   => __('Choose font file or ZIP of font files.', 'osmosis'),
                'media_button'  => __('Update', 'osmosis'),
            ) ) );            
