...
if ( !function_exists('grve_opengraph_add_prefix') ) {
	function grve_opengraph_add_prefix( $output ) {

		if ( grve_visibility( 'meta_opengraph' ) ) {
			$prefixes = array(
				'og' => 'http://ogp.me/ns#',
			);
			$prefixes = apply_filters( 'grve_opengraph_prefixes', $prefixes );
...
add_filter( 'language_attributes', 'grve_opengraph_add_prefix' );

if ( !function_exists('grve_print_meta_tags') ) {
	function grve_print_meta_tags() {
		...
		if ( grve_visibility( 'meta_viewport_responsive', '1' ) ) {
		...
		if ( grve_visibility( 'meta_opengraph' ) || grve_visibility( 'meta_twitter' ) ) {
			...
			if ( is_singular() || is_front_page() || is_home() ) {
				...
				//Twitter Cards
				if ( grve_visibility( 'meta_twitter' ) ) {
		...
		$meta_name_tags = apply_filters( 'grve_meta_name_tags', $meta_name_tags );
		$meta_property_tags = apply_filters( 'grve_meta_property_tags', $meta_property_tags );
...
add_action( 'wp_head', 'grve_print_meta_tags', 5 );
