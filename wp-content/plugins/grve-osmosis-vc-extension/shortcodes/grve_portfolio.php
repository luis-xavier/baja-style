...
<?php
		...
		while ( $query->have_posts() ) : $query->the_post();
			...
			if ( 'small-media' == $portfolio_style ) {
				...
				if ( 'popup' == $portfolio_link_type ) {
				?>
					<a href="<?php grve_osmosis_vce_print_portfolio_image( 'full', 'link' ); ?>">
				<?php
...
?>
					<article id="portfolio-<?php the_ID(); ?><?php echo uniqid('-'); ?>" <?php post_class( $portfolio_extra_class ); ?>>
									...
									<?php
										if ( 'popup' == $portfolio_link_type ) {
											...
											if ( 'yes' != $hide_portfolio_title  ) {
												$data_url .= ' data-title="' . esc_attr( get_the_title() ) . '"';
											}
											if ( !empty( $caption ) && 'yes' != $hide_portfolio_caption  ) {
												$data_url .= ' data-desc="' . esc_attr( $caption ) . '"';
											}
									?>
										<a title="<?php the_title(); ?>" class="grve-portfolio-btns" href="<?php grve_osmosis_vce_print_portfolio_image( 'full', 'link' ); ?>"<?php echo $data_url; ?>><?php echo $portfolio_link_type_title; ?></a>
									<?php
						...
						?>
							...
							<?php echo $link_end; ?>
							<?php do_action( 'grve_portfolio_hover_style_3_after' ); ?>
						<?php
						...
						?>
							...
							<?php echo $link_end; ?>
							<?php do_action( 'grve_portfolio_hover_style_2_after' ); ?>
						<?php
							} else {
						...
						?>
							...
							<?php echo $link_end; ?>
							<?php do_action( 'grve_portfolio_hover_style_1_after' ); ?>
						<?php
							}
						}
						?>
...
if( !function_exists( 'grve_osmosis_vce_portfolio_shortcode_params' ) ) {
	function grve_osmosis_vce_portfolio_shortcode_params( $tag ) {
		return array(
			...
			"params" => array(
				...
				array(
					"type" => "dropdown",
					"heading" => __( "Carousel Image Mode", "grve-osmosis-vc-extension" ),
					...
					'std' => 'landscape',
					"description" => __( "Select your Carousel Image Mode.", "grve-osmosis-vc-extension" ),
				...
				array(
					...
					"dependency" => Array( 'element' => "portfolio_hover_style", 'value' => array( 'hover-style-1', 'hover-style-3' ) ),
				),
				array(
					"type" => "dropdown",
...
if( function_exists( 'vc_lean_map' ) ) {
	vc_lean_map( 'grve_portfolio', 'grve_osmosis_vce_portfolio_shortcode_params' );
} else if( function_exists( 'vc_map' ) ) {
	$attributes = grve_osmosis_vce_portfolio_shortcode_params( 'grve_portfolio' );
	vc_map( $attributes );
}
