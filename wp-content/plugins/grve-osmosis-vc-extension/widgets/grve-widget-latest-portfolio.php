...
class Osmosis_Ext_Widget_Latest_Portfolio extends WP_Widget {

	function __construct() {
		$widget_ops = array(
			'classname' => 'grve-latest-portfolio',
			'description' => esc_html__( 'A widget that displays latest portfolio items.', 'grve-osmosis-vc-extension' ),
		);
		$control_ops = array(
			'width' => 300,
			'height' => 350,
		...
		parent::__construct( 'grve-widget-latest-portfolio', '(Greatives) ' . esc_html__( 'Latest Portfolio', 'grve-osmosis-vc-extension' ), $widget_ops, $control_ops );
	}

	function osmosis_ext_widget_latest_portfolio() {
		$this->__construct();
	}
	...
	function widget( $args, $instance ) {
		...
		?>
			...
			<?php esc_html_e( 'No Portfolio Items Found!', 'grve-osmosis-vc-extension' ); ?>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'num_of_posts' ) ); ?>"><?php esc_html_e( 'Number of Portfolio Items:', 'grve-osmosis-vc-extension' ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>"><?php esc_html_e( 'Categories:', 'grve-osmosis-vc-extension' ) ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'categories' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'categories' ) ); ?>[]" multiple="multiple" style="width:100%;">
				<option value=""><?php esc_html_e( 'Choose Categories ...', 'grve-osmosis-vc-extension' ) ?></option>
			<?php
				...
				?>
				<option value="<?php echo esc_attr( $category->term_id ); ?>" <?php selected( $category->term_id, $selected ); ?>><?php echo esc_html( $category->name ); ?></option>
			<?php
				}
			?>
			</select>
